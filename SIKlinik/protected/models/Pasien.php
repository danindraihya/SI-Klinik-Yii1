...
class Pasien extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('umur, status', 'numerical', 'integerOnly'=>true),
			array('nama', 'length', 'max'=>255),
			array('tanggal_periksa', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, nama, umur, status, tanggal_periksa', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'biayaObats' => array(self::HAS_MANY, 'BiayaObat', 'pasien_id'),
			'biayaTindakans' => array(self::HAS_MANY, 'BiayaTindakan', 'pasien_id'),
		);
	}
